...
if lc_zh_path == 'default':
    lc_zh_path = make_default_path('domain_'+domain_name+'/parameters/zonalhist/') # outputs a Path()
else:
...
if topo_path == 'default':
    topo_path = make_default_path('domain_'+domain_name+'/parameters//topology/') # outputs a Path()
else:
...
if merit_path == 'default':
    merit_path = make_default_path('domain_'+domain_name+'/shapefiles/catchment/') # outputs a Path()
else:
...
# Specify default path if needed
if outdir == 'default':
    outdir = make_default_path('domain_'+domain_name+'/parameters/drainagedatabase/') # outputs a Path()
else:
...
for i in (range(1,m)):
    ...
    if (fid.size == 0):
        ...
        notinLClist.append(str('land cover %s is not presented in the list of NALCMS land cover for this PFAF' % lc_type[i-1])) # This list will be appended to the log file at the end.
...
if str(input_lc_zh).endswith('.shp'):
    lc_frac = lc_frac.apply(lambda x: x/x.sum(),2, axis=1)
...
logPath = outdir
log_suffix = '_MESH_drainage_database.txt'
